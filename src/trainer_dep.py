...
from metrics.depth import Evaluator
...
class Trainer:
    ...
    def inputs_to_device(self, inputs):
        keys = inputs.keys()
        keys = [key for key in keys if "sem" not in key]
            ...
            if self.mini_step % self.args.accum_grads == 0:
                ...
                if self.step % self.args.log_frequency == 0:
                    with torch.no_grad():
                        ...
                        gt_depth = inputs["normalized_sphere_gt_depth"] * mask

                        errors = self.evaluator.compute_eval_metrics(gt_depth, pred_depth, mask, track=False)

                        self.log("train", losses, errors, best_errors=None)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(pbar):
                ...
                self.evaluator.compute_eval_metrics(gt_depth, pred_depth, mask, track=True)

        self.evaluator.print()

        errors, best_errors = self.evaluator.get_results()

        self.log("val", losses, errors, best_errors)

    def process_batch(self, inputs):
        x = inputs["normalized_sphere_rgb"]
        mask = inputs["sphere_valid_mask"]
        gt = inputs["normalized_sphere_gt_depth"]
        ...
        loss_rec = self.compute_loss(masked_pred, masked_gt)
        ...
        outputs = {
            ...
            "masked_pred_depth": masked_pred.detach(),
        }

        return outputs, losses

    def log(self, mode, losses, errors, best_errors=None):
